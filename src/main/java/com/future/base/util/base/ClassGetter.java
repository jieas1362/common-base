...
public final class ClassGetter {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ClassGetter.class);
    ...
    public static List<Class<?>> getClassesByPackage(String packageName, boolean recursive) {
        ...
        try {
                ...
                if (isNotNull(url)) {
                    ...
                    if (FILE_PROTOCOL.equals(protocol)) {
                        ...
                        try {
                            ...
                            LOGGER.error("getClassesByPackage(String packageName, boolean recursive) failed, Exception e = {0}", e);
                        }
            ...
            LOGGER.error("getClassesByPackage(String packageName, boolean recursive) failed, IOException e = {0}", e);
        }
        ...
        return clzNames.stream().filter(StringUtils::hasText)
                .map(cn -> {
                    try {
                        ...
                        LOGGER.error("getClassesByPackage(String packageName, boolean recursive) failed, ClassNotFoundException e = {0}", e);
                    }
                    return null;
    ...
    private static void handlePath(String path, List<String> clzNames) {
        ...
        if (path.contains(JAVA_MAIN_PREFIX)) {
            ...
            throw new ProException(BAD_REQUEST.status, BAD_REQUEST.code, "handlePath(String path, List<String> clzNames) failed, path = " + path);
        }
    ...
    private static List<String> getAllClassNameByJar(JarFile jarFile, String packageName, boolean recursive) {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (name.endsWith(CLASS_SUFFIX)) {
                name = name.replace(CLASS_SUFFIX, "").replace(PATH_SEPARATOR, PACKAGE_SEPARATOR);
                if (recursive) {
